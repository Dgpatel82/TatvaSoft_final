...
namespace Data_Access_Layer
{
    public class DALMission
    {
        ...
        public string AddMission(Missions mission)
        {
            ...
            mission.CityName = "";
            mission.CountryName = _cIDbContext.Country.Where(c => c.Id == mission.CountryId).Select(c => c.CountryName).FirstOrDefault();
            mission.CreatedDate = DateTime.UtcNow;
            mission.RegistrationDeadLine = mission.StartDate.Subtract(TimeSpan.FromDays(1));
            ...
            try
            {
                clientSideMissionList = _cIDbContext.Missions
                    ...
                    .Select(m => new Missions
                    {
                        ...
                        CountryName = _cIDbContext.Country.Where(c => c.Id == m.CountryId).Select(c => c.CountryName).FirstOrDefault(),
                        CityId = m.CityId,
                        CityName = _cIDbContext.City.Where(c => c.Id == m.CityId).Select(c => c.CityName).FirstOrDefault(),
                        ...
                        MissionThemeName = _cIDbContext.MissionTheme.Where(mt => m.MissionThemeId.Contains(mt.Id.ToString())).Select(mt => mt.ThemeName).FirstOrDefault(),
                        //MissionSkillName = string.Join(",", m.MissionSkillName),
                        MissionSkillName = _cIDbContext.MissionSkill.Where(ms => m.MissionSkillId.Contains(ms.Id.ToString())).Select(ms => ms.SkillName).FirstOrDefault(),
